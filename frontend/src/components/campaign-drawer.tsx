...
export function CampaignDrawer({
  ...
  return (
    <DrawerShell
      onClose={onClose}
      label="Kampanya ayrıntısı"
      // The carrier's own hex, not a category token: on this page the airline
      // IS the identity, and the seam light reads from here.
      glowColor={brandHex}
      className="max-w-md"
    >
      ...
      >
        ...
        {/* Status, class and the warning badges sit above everything else:
            ...
            reading at all. */}
        <motion.div variants={fadeUpItem} className="flex flex-wrap items-center gap-1">
        ...
        <motion.div variants={fadeUpItem} className="flex items-baseline gap-2">
        ...
        {markets.length > 0 && (
          <motion.div variants={fadeUpItem} className="flex flex-wrap gap-1">
            {markets.map((market) => (
              <span
                key={market}
